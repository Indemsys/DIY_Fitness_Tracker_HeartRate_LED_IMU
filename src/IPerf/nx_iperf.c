...
NX_WEB_HTTP_SERVER nx_iperf_web_server @ ".sdram";
FX_MEDIA           nx_iperf_ram_disk   @ ".sdram";
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param ip_ptr
  \param http_stack
  \param http_stack_size
  \param iperf_stack
  \param iperf_stack_size
-----------------------------------------------------------------------------------------------------*/
void    nx_iperf_entry(NX_PACKET_POOL *pool_ptr, NX_IP *ip_ptr, UCHAR *http_stack, ULONG http_stack_size, UCHAR *iperf_stack, ULONG iperf_stack_size)
{
  UINT status;

   APPLOG("Iperf task entry");
  /* Create the HTTP Server.  */
  status =  nx_web_http_server_create(&nx_iperf_web_server, "IPerf HTTP Server", ip_ptr, NX_WEB_HTTP_SERVER_PORT,&nx_iperf_ram_disk, http_stack, http_stack_size, pool_ptr, nx_iperf_authentication_check, nx_iperf_get_notify);

  /* Check the status.  */
  if (status)
  {
    APPLOG("Iperf task fail. Error creating WEB server: %d", status);
  ...
  if (status)
  {
    APPLOG("Iperf task fail. Error staring WEB server: %d", status);

    /* Update the error counter and return.  */
    nx_iperf_test_error_counter++;
    return;
  }
  APPLOG("Iperf task started successfully");
}
...
void    nx_iperf_thread_tcp_tx_entry(ULONG thread_input)
{
  ...
  while (tx_time_get() < expire_time)
  {
    ...
    packet_num++;
    memcpy(my_packet->nx_packet_prepend_ptr+4, &packet_num, 4);
