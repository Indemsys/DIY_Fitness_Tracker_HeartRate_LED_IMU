...
output_c_file = '..\..\src\WEB\Test_WEB_files.c'
output_h_file = '..\..\src\WEB\Test_WEB_files.h'
...
with open(output_h_file, 'w') as f:
    f.write('#ifndef _APP_COMPRESSED_WEB_FILES_H\n')
    f.write('  #define _APP_COMPRESSED_WEB_FILES_H\n\n')
    f.write('typedef struct {\n')
    f.write('    const char* name;\n')
    f.write('    const uint8_t* data;\n')
    f.write('    uint32_t size;\n')
    f.write('} T_app_compressed_file_rec ;\n\n')
    f.write(f'#define APP_COMPRESSED_WEB_FILES_NUM {files_cnt}\n\n') 
    f.write(f'extern const T_app_compressed_file_rec app_compressed_web_files[APP_COMPRESSED_WEB_FILES_NUM];\n\n') 
    f.write('#endif\n')
...
with open(output_c_file, 'w') as f:
    ...
    for file, data in compressed_files_data.items():
        ...
        array_size = len(data)
        array_name = 'F%d_%s_data' % (cnt, array_name)
    ...
    cnt = 0; 
    f.write(f'const T_app_compressed_file_rec app_compressed_web_files[APP_COMPRESSED_WEB_FILES_NUM] = ')
    f.write('{\n')
    for file, data in compressed_files_data.items():
