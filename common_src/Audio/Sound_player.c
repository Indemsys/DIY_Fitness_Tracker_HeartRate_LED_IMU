...
#define PLAYER_MESAGGES_QUEUE_SZ  100
TX_QUEUE               player_queue;
T_player_msg           player_queue_buf[PLAYER_MESAGGES_QUEUE_SZ];


/*-----------------------------------------------------------------------------------------------------
  Включение и выключение аудиусилителя на плате

-----------------------------------------------------------------------------------------------------*/
__attribute__((weak))  void  Set_AUD_SHDN(int32_t v)
{
...
{
  ply.voice_language = val;
}


/*-----------------------------------------------------------------------------------------------------
  Обработка управляющих сообщений задаче проигрывателя

-----------------------------------------------------------------------------------------------------*/
__attribute__((weak)) void VAnnouncer_messages_processing(const uint8_t *buf)
{
...
{
  return (MAX_SOUND_VOLUME-ply.attenuation);
}
...
static int _Audio_init_play_file(void)
{
  ...
  if (ply.block_sz <= 0)
  {
    EAPPLOG("Reading file %s error.", ply.file_name);
    goto err_exit;
  }
...
static uint32_t _Audio_wait_event(uint32_t ticks)
{
  ...
  if (tx_event_flags_get(&play_flags, 0xFFFFFFFF, TX_OR_CLEAR ,&actual_flags, ticks) == TX_SUCCESS)
  {
    ...
    if (actual_flags &  EVT_MSG_IN_QUEUE)
    {
...
/*-----------------------------------------------------------------------------------------------------
  Передача сообщения от кнопок на плате в очередь наблюдаемую задачей управления
-----------------------------------------------------------------------------------------------------*/
...
{
  tx_event_flags_set(&play_flags, EVT_MSG_IN_QUEUE, TX_OR);
}
...
static void SoundPlayer_task(ULONG arg)
{
  ...
  Set_app_event(EVENT_PLAYER_TASK_READY);

  Player_Set_Volume(ivar.audio_volume);
...
uint32_t  Create_Sound_Player_task(void)
{
  ...
  if (err == TX_SUCCESS)
  {
    EAPPLOG("Sound player task created.");
    return RES_OK;
  }
  else
  {
    EAPPLOG("Sound player creating error %d.", err);
    return RES_ERROR;
  }
