                ...
                for(p = (NX_MDNS_RR*)((UCHAR*)mdns_ptr -> nx_mdns_local_service_cache + sizeof(ULONG)); (ULONG*)p < head; p++)
                {
                        ...
                        (p -> nx_mdns_rr_state == NX_MDNS_RR_STATE_ANNOUNCING))
                    {
                        ...
                        else
                        {
                            ...
                            if (authority_count)
                            {
                                int32_t d = NX_MDNS_RESPONSE_INTERVAL - NX_MDNS_RESPONSE_PROBING_TIMER_COUNT;
                                if (d < 0) d = 0; 
                                if (p -> nx_mdns_rr_response_interval > (uint32_t)d)
                                    p -> nx_mdns_rr_response_interval = (ULONG)(p -> nx_mdns_rr_response_interval - d);
                                else
                                    p -> nx_mdns_rr_response_interval = 0;
                            }                                 
