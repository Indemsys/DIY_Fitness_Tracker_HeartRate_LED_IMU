...
/*                                                                        */
/**************************************************************************/
static VOID nx_bsd_timeout_process(void)
{
...
INT  bind(INT sockID, struct sockaddr *localAddress, INT addressLength)
{
  ...
  if (bsd_socket_ptr->nx_bsd_socket_tcp_socket)
  {
    ...
    /* Update the port. */
    if ((status == NX_SUCCESS) && (local_port == 0)) local_port = (INT)(tcp_socket_ptr->nx_tcp_socket_port);
  ...
  else if (bsd_socket_ptr->nx_bsd_socket_udp_socket)
  {
    ...
    /* Update the port. */
    if ((status == NX_SUCCESS) && (local_port == 0)) local_port = (INT)(udp_socket_ptr->nx_udp_socket_port);
  }
  else
  {
                                ...
                                NXD_ADDRESS *dst_address, USHORT dst_port, UINT local_interface_index)
{
  ...
  else wait_option = TX_WAIT_FOREVER;
  ...
  if (bsd_socket_ptr->nx_bsd_socket_protocol == NX_PROTOCOL_UDP)
  {
    ...
    /* Send the UDP packet.  */
    if (local_interface_index == NX_BSD_LOCAL_IF_INADDR_ANY) status =  nxd_udp_socket_send(udp_socket_ptr, packet_ptr, dst_address, dst_port);
    else status =  nxd_udp_socket_interface_send(udp_socket_ptr, packet_ptr, dst_address, dst_port, local_interface_index);
  }
  else if (bsd_socket_ptr->nx_bsd_socket_protocol == NX_PROTOCOL_TCP)
  {
  ...
  else
  {
    ...
    if (bsd_socket_ptr->nx_bsd_socket_status_flags & NX_BSD_SOCKET_TX_HDR_INCLUDE)
    {
      ...
      if (bsd_socket_ptr->nx_bsd_socket_family == AF_INET)
      {
        ...
        if (src_interface == NX_BSD_LOCAL_IF_INADDR_ANY) src_interface = 0;
    ...
    else
    {
      ...
      if (local_interface_index == NX_BSD_LOCAL_IF_INADDR_ANY) local_interface_index = 0;
  ...
  if (status != NX_SUCCESS)
  {
    ...
    case NX_TX_QUEUE_DEPTH:
      if ((bsd_socket_ptr->nx_bsd_socket_option_flags & NX_BSD_SOCKET_ENABLE_OPTION_NON_BLOCKING) ||
          (flags & MSG_DONTWAIT)) set_errno(EWOULDBLOCK);
      else set_errno(ETIMEDOUT);
      break;
      ...
      if (bsd_socket_ptr->nx_bsd_socket_family == AF_INET6)
      {
        ...
        if (bsd_socket_ptr->nx_bsd_socket_family == AF_PACKET)
        {
          if (*fromAddrLen >= (INT)sizeof(struct sockaddr_ll))
          {
            ...
            for (i = 0; i < 6; i++) sockaddr->sll_addr[i] = bsd_socket_ptr->nx_bsd_socket_sll_addr[i];
...
UINT  bsd_number_convert(UINT number, CHAR *string, ULONG buffer_len, UINT base)
{
  ...
  while (size < buffer_len)
  {
    ...
    if (digit < 10) string[0] =  (CHAR)(digit + 0x30);
    else string[0] =  (CHAR)(digit + 0x57);
...
INT  setsockopt(INT sockID, INT option_level, INT option_name, const VOID *option_value, INT option_length)
{
  ...
  case SO_RCVBUF:
    ...
    if (window_size > 65535) window_size = 65535;
  ...
  case SO_REUSEADDR:
    ...
    if (reuse_enabled) bsd_socket_ptr->nx_bsd_socket_option_flags |= NX_BSD_SOCKET_ENABLE_OPTION_REUSEADDR;
    else bsd_socket_ptr->nx_bsd_socket_option_flags &= (ULONG)(~NX_BSD_SOCKET_ENABLE_OPTION_REUSEADDR);
...
INT  select(INT nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds, struct timeval *timeout)
{
  ...
  if (readfds) readfds_left = readfds->fd_count;
  else readfds_left = 0;
  ...
  for (i = 0; i < (nfds - NX_BSD_SOCKFD_START); i++)
  {
    ...
    if ((readfds == NX_NULL) || (readfds_left == 0)) break;
  ...
  for (i = 0; i < (nfds - NX_BSD_SOCKFD_START); i++)
  {
    ...
    if ((writefds == NX_NULL) || (writefds_left == 0)) break;
  ...
  for (i = 0; i < (nfds - NX_BSD_SOCKFD_START); i++)
  {
    ...
    if ((exceptfds == NX_NULL) || (exceptfds_left == 0)) break;
  ...
  if (readfds_found.fd_count || writefds_found.fd_count || exceptfds_found.fd_count)
  {
    ...
    if (readfds) *readfds =  readfds_found;

    if (writefds) *writefds = writefds_found;

    if (exceptfds) *exceptfds = exceptfds_found;
  ...
  if (readfds) suspend_request.nx_bsd_socket_suspend_read_fd_set =  *readfds;
  else FD_ZERO(&suspend_request.nx_bsd_socket_suspend_read_fd_set);

  if (writefds) suspend_request.nx_bsd_socket_suspend_write_fd_set = *writefds;
  else FD_ZERO(&suspend_request.nx_bsd_socket_suspend_write_fd_set);

  if (exceptfds) suspend_request.nx_bsd_socket_suspend_exception_fd_set = *exceptfds;
  else FD_ZERO(&suspend_request.nx_bsd_socket_suspend_exception_fd_set);
  ...
  if (status != NX_SUCCESS)
  {
    ...
    if (status == TX_NO_EVENTS)
    {
      ...
      if (ticks == 0) set_errno(EWOULDBLOCK);
      else set_errno(ETIMEDOUT);
  ...
  else
  {
    ...
    if (readfds) *readfds =  suspend_request.nx_bsd_socket_suspend_read_fd_set;
    if (writefds) *writefds = suspend_request.nx_bsd_socket_suspend_write_fd_set;
    if (exceptfds) *exceptfds = suspend_request.nx_bsd_socket_suspend_exception_fd_set;
...
UINT  nx_bsd_raw_packet_info_extract(NX_PACKET *packet_ptr, NXD_ADDRESS *address, UINT *interface_index)
{
  ...
  if (interface_index == NX_NULL) return (NX_SUCCESS);
...
UINT  nx_packet_data_extract_offset(NX_PACKET *packet_ptr, ULONG offset, VOID *buffer_start, ULONG buffer_length, ULONG *bytes_copied)
{
  ...
  if (source_ptr == NX_NULL) return (NX_PACKET_OFFSET_ERROR);
  ...
  while (working_packet_ptr && remaining_bytes)
  {
    ...
    if (remaining_bytes < bytes_to_copy) bytes_to_copy = remaining_bytes;
...
UINT nx_bsd_socket_set_inherited_settings(UINT master_sock_id, UINT secondary_sock_id)
{
  ...
  if (nx_bsd_socket_array[master_sock_id].nx_bsd_socket_option_flags & NX_BSD_SOCKET_ENABLE_OPTION_NON_BLOCKING) nx_bsd_socket_array[secondary_sock_id].nx_bsd_socket_option_flags |= NX_BSD_SOCKET_ENABLE_OPTION_NON_BLOCKING;
  else nx_bsd_socket_array[secondary_sock_id].nx_bsd_socket_option_flags &= (ULONG)(~NX_BSD_SOCKET_ENABLE_OPTION_NON_BLOCKING);

  if (nx_bsd_socket_array[master_sock_id].nx_bsd_socket_option_flags & NX_BSD_SOCKET_ENABLE_OPTION_REUSEADDR) nx_bsd_socket_array[secondary_sock_id].nx_bsd_socket_option_flags |= NX_BSD_SOCKET_ENABLE_OPTION_REUSEADDR;
  else nx_bsd_socket_array[secondary_sock_id].nx_bsd_socket_option_flags &= (ULONG)(~NX_BSD_SOCKET_ENABLE_OPTION_REUSEADDR);
...
static VOID nx_bsd_udp_packet_received(INT sockID, NX_PACKET *packet_ptr)
{
  ...
  do
  {
    ...
    if (bsd_ptr->nx_bsd_socket_family == addr_family)
    {
      ...
      if (bsd_ptr->nx_bsd_socket_status_flags & NX_BSD_SOCKET_CONNECTED)
      {
        ...
        if (exact_match != NX_NULL) break;

        if (receiver_match != NX_NULL) receiver_match = NX_NULL;

        if (wildcard_match != NX_NULL) wildcard_match = NX_NULL;
      }
    }
  ...
  if (exact_match != NX_NULL) bsd_ptr = exact_match;
  else if (receiver_match != NX_NULL) bsd_ptr = receiver_match;
  else if (wildcard_match != NX_NULL) bsd_ptr = wildcard_match;
  else
  {
...
static UINT  nx_bsd_tcp_syn_received_notify(NX_TCP_SOCKET *socket_ptr, NX_PACKET *packet_ptr)
{
  ...
  if (packet_ptr->nx_packet_ip_version == NX_IP_VERSION_V4)
    ...
    interface_ptr = packet_ptr->nx_packet_address.nx_packet_interface_ptr;
  #ifdef FEATURE_NX_IPV6
  else if (packet_ptr->nx_packet_ip_version == NX_IP_VERSION_V6) interface_ptr = packet_ptr->nx_packet_address.nx_packet_ipv6_address_ptr->nxd_ipv6_address_attached;
  else return (NX_FALSE);
  #endif /* FEATURE_NX_IPV6 */
  ...
  for (i = 0; i < NX_BSD_MAX_SOCKETS; i++)
  {
    ...
    if (search_index >= NX_BSD_MAX_SOCKETS) search_index = 0;
  }

  if (receiver_match != NX_BSD_MAX_SOCKETS) sockID_find = receiver_match;
  else if (wildcard_match != NX_BSD_MAX_SOCKETS) sockID_find = wildcard_match;
  else
  {
...
static INT nx_bsd_tcp_create_listen_socket(INT master_sockid, INT backlog)
{
  ...
  for (i = 0; i < NX_BSD_MAX_SOCKETS; i++)
  {
    ...
    if ((nx_bsd_socket_array[i].nx_bsd_socket_status_flags & NX_BSD_SOCKET_SERVER_MASTER_SOCKET) == 0) continue;

    /* Skip the current master socket. */
    if (i == master_sockid) continue;

    /* Skip the entry if it is not TCP */
    if (nx_bsd_socket_array[i].nx_bsd_socket_protocol != NX_PROTOCOL_TCP) continue;

    /* Skip the entry if the the secondary socket id field is not valid. */
    if (nx_bsd_socket_array[i].nx_bsd_socket_union_id.nx_bsd_socket_secondary_socket_id == NX_BSD_MAX_SOCKETS) continue;
  ...
  if (backlog) status = nx_tcp_server_socket_listen(nx_bsd_default_ip, local_port, bsd_secondary_socket->nx_bsd_socket_tcp_socket, (UINT)backlog, NX_NULL);
  else
  {
  ...
  if (addr_family == AF_INET)
  {
    ...
    for (i = 0; i < NX_MAX_IP_INTERFACES; i++)
    {
      if ((nx_bsd_default_ip->nx_ip_interface[i].nx_interface_valid) &&
          (nx_bsd_default_ip->nx_ip_interface[i].nx_interface_ip_address == ipv4_addr)) return i;
    }
  }
  #endif /* NX_DISABLE_IPV4 */
  #ifdef FEATURE_NX_IPV6
  if (addr_family == AF_INET6)
  {
    ...
    for (i = 0; i < NX_MAX_IPV6_ADDRESSES; i++)
    {
      ...
      if ((nx_bsd_default_ip->nx_ipv6_address[i].nxd_ipv6_address_valid) &&
          ...
          (nx_bsd_default_ip->nx_ipv6_address[i].nxd_ipv6_address[3] == ipv6_addr[3])) return i;
    }
  }
  #endif
  ...
  if (if_ptr && if_ptr->nx_interface_address_mapping_needed)
  {
    ...
    if (status != NX_SUCCESS)
    {
      ...
      if (_nxd_ipv6_search_onlink(ip_ptr, dest_addr))
      {
        ...
        if (status == NX_SUCCESS) NDCacheEntry = dest_entry_ptr->nx_ipv6_destination_entry_nd_entry;
      }
      ...
      else if (_nxd_ipv6_router_lookup(ip_ptr, if_ptr, next_hop_address, (VOID **)&NDCacheEntry) == NX_SUCCESS)
      {
        ...
        if (!NDCacheEntry) NDCacheEntry = dest_entry_ptr->nx_ipv6_destination_entry_nd_entry;
      }
...
static VOID  _nxd_bsd_swap_ipv6_extension_headers(NX_PACKET *packet_ptr, UCHAR header_type)
{
  ...
  while (remaining_bytes)
  {
    ...
    if (header_type == NX_PROTOCOL_NEXT_HEADER_AUTHENTICATION) header_length = (ULONG)((option->nx_ipv6_header_option_ext_length << 2) + 2);
    else header_length = (ULONG)((option->nx_ipv6_header_option_ext_length + 1) << 3);
...
static INT nx_bsd_pppoe_internal_sendto(NX_BSD_SOCKET *bsd_socket_ptr, CHAR *msg, INT msgLength, INT flags,  struct sockaddr *destAddr, INT destAddrLen)
{
  ...
  if (destAddr_ll->sll_protocol == ETHERTYPE_PPPOE_SESS) driver_request.nx_ip_driver_command =           NX_LINK_PACKET_PPPOE_SESS_SEND;
  else driver_request.nx_ip_driver_command =           NX_LINK_PACKET_PPPOE_DISC_SEND;
  driver_request.nx_ip_driver_packet =                packet_ptr;
...
INT  inet_pton(INT af, const CHAR *src, VOID *dst)
{
  ...
  else if (af == AF_INET6)
  {
    ...
    while (*src != 0)
    {
       ...
       */
      minuend = 0;
      if (ch >= '0' && ch <= '9') minuend = 48;
      else if (ch >= 'a' && ch <= 'f') minuend = 87;
      else if (ch >= 'A' && ch <= 'F') minuend = 55;
    ...
    if (digit_counter)
    {
      ...
      /* Invalid Presentation Format. */
      if (dst_cur_ptr + 1 > dst_end_ptr) return 0;
    ...
    if (colon_location)
    {
      ...
      if (dst_cur_ptr == dst_end_ptr) return 0;
    ...
    if (dst_cur_ptr != (dst_end_ptr + 1)) return 0;
...
const CHAR* inet_ntop(INT af, const VOID *src, CHAR *dst, socklen_t size)
{
  ...
  if (af == AF_INET)
  {
    ...
    if (inet_ntoa_internal(src, dst, size)) return dst;
    else return NX_NULL;
  }
  else if (af == AF_INET6)
  {
    ...
    for (i = 0; i < 8; i++)
    {
          ...
          (i < (shorthand_index + shorthand_len)))
      {
        if (i == shorthand_index)
        {
          ...
          if ((size - (ULONG)index) < 1) return NX_NULL;
...
INT  getaddrinfo(const CHAR *node, const CHAR *service, const struct addrinfo *hints, struct addrinfo **res)
{
  ...
  if ((node == NX_NULL) && (service == NX_NULL)) return EAI_NONAME;
  ...
  if (hints)
  {
    ...
    if ((hints->ai_family != AF_INET) &&
        ...
        (hints->ai_family != AF_UNSPEC)) return EAI_FAMILY;

    /* Check if the socket type is valid. */
    if ((hints->ai_socktype != SOCK_DGRAM) &&
        (hints->ai_socktype != SOCK_STREAM) &&
        (hints->ai_socktype != 0)) return EAI_SOCKTYPE;
    ...
    if ((hints->ai_socktype != 0) && (hints->ai_protocol != 0))
    {
      if (((hints->ai_socktype == SOCK_STREAM) && (hints->ai_protocol != IPPROTO_TCP)) ||
          ...
          ((hints->ai_socktype == SOCK_RAW) && (hints->ai_protocol != IPPROTO_RAW))) return EAI_SOCKTYPE;
    }
  ...
  if (hints->ai_socktype == 0)
  {
    ...
    else
    {
      ...
      if (hints->ai_protocol == IPPROTO_TCP) match_service_socktype[0] = SOCK_STREAM;
      else if (hints->ai_protocol == IPPROTO_UDP) match_service_socktype[0] = SOCK_DGRAM;
    }
  }
  else
  {
    ...
    if (hints->ai_protocol == 0)
    {
      ...
      if (hints->ai_socktype == SOCK_STREAM) match_service_protocol[0] = IPPROTO_TCP;
      else if (hints->ai_socktype == SOCK_DGRAM) match_service_protocol[0] = IPPROTO_UDP;
      else if (hints->ai_socktype == SOCK_RAW) match_service_protocol[0] = 0;
    }
    else
    {
  ...
  if (service)
  {
    ...
    else
    {
      ...
      if (hints->ai_flags & AI_NUMERICSERV) return EAI_NONAME;
      ...
      if (match_service_count == 0) return EAI_SERVICE;
    }
  }
  else
  {
  ...
  if (node)
  {
    ...
    if (addr_family == AF_INET)
    {
      ...
      if (inet_pton(addr_family, node,&(nx_bsd_ipv4_addr_buffer[0])) == 1)
      {
        ...
        if ((hints->ai_family != AF_INET) && (hints->ai_family != AF_UNSPEC)) return EAI_ADDRFAMILY;
    ...
    else
    {
      ...
      if (inet_pton(addr_family, node,&(nx_bsd_ipv6_addr_buffer[0])) == 1)
      {
        ...
        if ((hints->ai_family != AF_INET6) && (hints->ai_family != AF_UNSPEC)) return EAI_ADDRFAMILY;
    ...
    if (pton_flag == 1)
    {
      ...
      if ((hints->ai_flags & AI_CANONNAME) && !(hints->ai_flags & AI_NUMERICHOST))
      {
        ...
        /* Verify buffer size. */
        if (_nx_bsd_string_length((CHAR *)node) > NX_DNS_NAME_MAX) return (EAI_OVERFLOW);
    ...
    else
    {
      ...
      if (hints->ai_flags & AI_NUMERICHOST) return EAI_NONAME;
  ...
  for (i = 0; i < ipv4_addr_count + ipv6_addr_count; i++)
  {
    ...
    if (status != TX_SUCCESS)
    {
      ...
      if (addrinfo_head_ptr) freeaddrinfo(addrinfo_head_ptr);

      #ifdef NX_DNS_ENABLE_EXTENDED_RR_TYPES
      if (hints->ai_flags & AI_CANONNAME) tx_block_release((VOID *)cname_buffer);
      #endif
    ...
    for (j = 0; j < match_service_count; j++)
    {
      ...
      if ((i == 0) && (j == 0) && (hints->ai_flags & AI_CANONNAME)) addrinfo_cur_ptr->ai_canonname = (CHAR *)cname_buffer;
      else addrinfo_cur_ptr->ai_canonname = NX_NULL;
      addrinfo_cur_ptr->ai_next = NX_NULL;

      /* Make a list. */
      if (addrinfo_head_ptr == NX_NULL) addrinfo_head_ptr = addrinfo_cur_ptr;
      else addrinfo_tail_ptr->ai_next = addrinfo_cur_ptr;
...
static INT bsd_string_to_number(const CHAR *string, UINT *number)
{
  ...
  *number = 0;
  ...
  while (*string != '\0')
  {
    ...
    if (!nx_bsd_isdigit((UCHAR)(*string))) return NX_SOC_ERROR;
...
INT  getnameinfo(const struct sockaddr *sa, socklen_t salen, char *host, size_t hostlen, char *serv, size_t servlen, int flags)
{
  ...
  if ((host == NX_NULL) && (serv == NX_NULL) && ((flags & NI_NAMEREQD) == 0)) return EAI_NONAME;
  ...
  if (serv && servlen > 0)
  {
    ...
    if (numeric_flag)
    {
      ...
      temp = (USHORT *)(sa->sa_data);
      if (bsd_number_convert(htons(*temp), (CHAR *)serv, servlen, 10) == 0) return EAI_OVERFLOW;
    }
    else
    {

      /* Service isn't numeric, copy the service name. */
      if (_nx_bsd_string_length(_nx_bsd_serv_list_ptr[i].service_name) > servlen) return EAI_OVERFLOW;
  ...
  if (host && hostlen > 0)
  {
    ...
    if (flags & NI_NUMERICHOST) numeric_flag = 1;
    else
    {
      ...
      if (status == NX_DNS_SIZE_ERROR) return EAI_OVERFLOW;
      else if (status != NX_SUCCESS)
      {
    ...
    if (numeric_flag)
    {
      ...
      if (sa->sa_family == AF_INET) rt_ptr = inet_ntop(AF_INET,&((struct sockaddr_in *)sa)->sin_addr, (CHAR *)host, hostlen);
      else rt_ptr = inet_ntop(AF_INET6,&((struct sockaddr_in6 *)sa)->sin6_addr, (CHAR *)host, hostlen);

      if (!rt_ptr) return EAI_OVERFLOW;
    }
