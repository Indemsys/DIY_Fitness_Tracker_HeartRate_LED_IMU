...
#define SEM_NAME_PREFIX "semaphore_pipe_"
static CHAR semaphores_names[UX_DCD_SYNERGY_MAX_PIPE-UX_DCD_SYNERGY_MIN_PIPE+1][sizeof(SEM_NAME_PREFIX)+ 3];
...
#if    !defined(BSP_MCU_GROUP_S124) && !defined(BSP_MCU_GROUP_S128) && !defined(BSP_MCU_GROUP_S1JA)
  /** Create the semaphore (as this cannot be done in endpoint_create.c as it is called from interrupt context) */
  for (ULONG pipenum= UX_DCD_SYNERGY_MIN_PIPE; pipenum <= UX_DCD_SYNERGY_MAX_PIPE; pipenum++)
  {
    uint32_t indx = pipenum-UX_DCD_SYNERGY_MIN_PIPE;
    /* Convert the pipe number to human readable ASCII format */
    strcpy(semaphores_names[indx], SEM_NAME_PREFIX);
    uint32_t n = strlen(semaphores_names[indx]);
    semaphores_names[indx][n] = (CHAR)(pipenum + (ULONG)'0');
    semaphores_names[indx][n+1] = 0;

    /* ux_dcd_synergy_ep_slave_transfer_request_semaphore is used for flagging the completion of the transfer */
    semaphore_create_status =  _ux_utility_semaphore_create(&(dcd_synergy->ux_dcd_synergy_ed[(UINT)pipenum].ux_dcd_synergy_ep_slave_transfer_request_semaphore), semaphores_names[indx], 0);
