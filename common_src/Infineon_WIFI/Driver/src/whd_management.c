  ...
  if (bsscfgidx < WHD_INTERFACE_MAX)
  {
    ...
    if ((ifp = (whd_interface_t)whd_mem_malloc(sizeof(struct whd_interface))) != NULL)
    {
      ...
      *ifpp = ifp;
      ...
      if (mac_addr != NULL) memcpy(ifp->mac_addr.octet, mac_addr->octet, sizeof(whd_mac_t));
      else memset(ifp->mac_addr.octet, 0, sizeof(whd_mac_t));
...
whd_result_t whd_add_primary_interface(whd_driver_t whd_driver, whd_interface_t *ifpp,  whd_mac_t *mac_addr)
{
  WPRINT_WHD_DATA_LOG(("FUNC: whd_add_primary_interface\n"));
  return whd_add_interface(whd_driver, 0, 0, "wlan0", mac_addr, ifpp);
}
                  ...
                  whd_netif_funcs_t *network_ops)
{
  ...
  if ((whd_drv = (whd_driver_t)whd_mem_malloc(sizeof(struct whd_driver))) != NULL)
  {
    ...
    *whd_driver_ptr = whd_drv;
    ...
    if (whd_init_config->country == 0) whd_drv->country = WHD_COUNTRY_UNITED_STATES;
    else whd_drv->country = whd_init_config->country;
  }
  else
  {
    return WHD_MALLOC_FAILURE;
                                               ...
                                               whd_bool_t resume_after_deep_sleep)
{
  ...
  WPRINT_WHD_DATA_LOG(("FUNC: whd_management_wifi_platform_init\n"));
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param whd_driver
  \param ifpp
  \param mac_addr  - Добавленый параметр к оригинальному API. Можно передавать NULL

  \return uint32_t
-----------------------------------------------------------------------------------------------------*/
uint32_t whd_wifi_on(whd_driver_t whd_driver, whd_interface_t *ifpp, whd_mac_t *mac_addr)
{
  ...
  WPRINT_WHD_DATA_LOG(("FUNC: whd_wifi_on\n"));
  ...
  whd_add_primary_interface(whd_driver, ifpp, mac_addr);
  ifp = *ifpp;
...
uint32_t whd_wifi_off(whd_interface_t ifp)
{
  ...
  {
    whd_result_t check_res = whd_wifi_set_ioctl_buffer(ifp, WLC_DOWN, NULL, 0);
    if (check_res != WHD_SUCCESS)
    {
      WPRINT_WHD_ERROR(("Function %s failed at line %d checkres = %u \n", __func__, __LINE__, (unsigned int)check_res));
      return check_res;
    }
  }
