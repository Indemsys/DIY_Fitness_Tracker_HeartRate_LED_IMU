...
uint32_t Compress_file_to_file(int alg, char *infname, char *outfname)
{
  ...
  ptr1 = COMPRESSOR_MALLOC_PENDING((MAX_COMPRESSIBLE_BLOCK_SIZE * 2)+SPARE_AREA_SIZE, 10);
  if (ptr1 == NULL)
  {
    EAPPLOG("Malloc error");
    return RES_ERROR;
  }
  ...
  while (fsize > 0)
  {
    ...
    if (alg == COMPR_ALG_SIXPACK)
    {
      res = Compress_sixp(&inFP,&outFP); // Компрессор SIXP требует 65127 динамической памяти
    }
    else
    {
      res = Compress_lzss(&inFP,&outFP); // Компрессор LZSS требует 54301 динамической памяти
    }
...
uint32_t Decompress_file_to_file(int alg, char *infname, char *outfname)
{
  ...
  while (fsize > 0)
  {
    ...
    if (alg == COMPR_ALG_SIXPACK)
    {
      sz_decompr = Uncompress_sixp(&inFP,&outFP); // Декомпрессор SIXP требует 10411 байт динамической памяти
    }
    else
    {
      sz_decompr = Uncompress_lzss(&inFP,&outFP); //  Декомпрессор LZSS требует 4331 байт динамической памяти
    }
    ...
    {
      status = RES_ERROR;
      EAPPLOG("Error. Size=%d ", sz_decompr);
    }
...
uint32_t Decompress_file_to_mem(int alg, char *input_file_name, void *out_buf, unsigned int out_buf_sz)
{
  ...
  input_file = COMPRESSOR_MALLOC_PENDING(sizeof(FX_FILE), 10);
  ...
  compressed_data_buf = COMPRESSOR_MALLOC_PENDING(compressed_data_size, 10);
  ...
  if (alg == COMPR_ALG_SIXPACK)
  {
    res = Uncompress_sixp(&inFP,&outFP); // Декомпрессор SIXP требует 10411 байт динамической памяти
  }
  else
  {
    res = Uncompress_lzss(&inFP,&outFP); //  Декомпрессор LZSS требует 4331 байт динамической памяти
  }
  ...
  COMPRESSOR_MEM_FREE(compressed_data_buf);
  COMPRESSOR_MEM_FREE(input_file);
  return status;
