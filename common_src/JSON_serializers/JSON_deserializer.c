...
uint32_t JSON_Deser_tables(uint8_t ptype, json_t *root)
{
  ...
  if (err > 0)
  {
    EAPPLOG("Deserialization fail count = %d.", err);
    return RES_ERROR;
  }
  else
  {
    EAPPLOG("Deserialization done sucessfully.");
    return RES_OK;
  }
...
static  uint32_t _JSON_Deser_params(uint8_t ptype, json_t  *root)
{
  ...
  if (p_pars==NULL) return RES_ERROR;
  ...
  if (hits_cnt > 0)
  {
    EAPPLOG("Updated %d params.",hits_cnt);
  }
...
uint32_t JSON_Deser_settings(uint8_t ptype, char *text)
{
  ...
  res = JSON_Deser_tables(ptype, root);
  json_decref(root);
...
uint32_t JSON_Deser_and_Exec_command(char *text, uint32_t f_long_stream)
{
  ...
  if (res != RES_OK)
  {
    EAPPLOG("JSON decoding error: 'OpCode' key is not a string.");
    json_decref(root);
    return RES_ERROR;
  }
  ...
  case OPCODE_SAVE_PARAMETERS_TO_RAM:
    EAPPLOG("MQTT command: SAVE_PARAMETERS_TO_RAM");
    res = JSON_Deser_tables(MODULE_PARAMS, root);
    MQTTMC_Send_Ack(res);
    break;
  case OPCODE_SAVE_PARAMETERS_TO_NV:
    EAPPLOG("MQTT command: SAVE_PARAMETERS_TO_NV");
    res = JSON_Deser_tables(MODULE_PARAMS,root);
    MQTTMC_Send_Ack(res);
    if (res == RES_OK) Request_save_nv_parameters(MODULE_PARAMS);
    break;
