...
typedef struct
{
    ...
    uint32_t       sdram_free;
    uint32_t       up_time;
    uint32_t       total_up_time;
    uint32_t       sd_card_fs_state;
    uint8_t        has_intenet_connection;
} T_dev_status_snapshot;
...
static void  _Print_DeviceInfo(char *buff, uint32_t *buff_sz)
{
  ...
  offset += Print_to(buff, offset, "\"BootloaderVer\":\"%s\",", Get_bootloader_date_time());
  offset += Print_to(buff, offset, "\"Reboot_cnt\":%d",g_nv_cnts.sys.reboot_cnt);
  offset += Print_to(buff, offset, "}");
  *buff_sz = offset;
}
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param buff_sz
-----------------------------------------------------------------------------------------------------*/
static void  _Print_DeviceStatus(T_dev_status_snapshot *d_p, char *buff, uint32_t *buff_sz)
{
  uint32_t offset = 0;
  offset += Print_to(buff, offset, "{");
  ...
  offset += Print_to(buff, offset, "\"sta_rssi\":%d,",d_p->sta_rssi);
  ...
  offset += Print_to(buff, offset, "\"total_up_time\":%d,",d_p->total_up_time);
  offset += Print_to(buff, offset, "\"sd_card_fs_state\":%d,",d_p->sd_card_fs_state);
  offset += Print_to(buff, offset, "\"has_intenet_connection\":%d",d_p->has_intenet_connection);
  offset += Print_to(buff, offset, "}");
  *buff_sz = offset;
}
...
{
  T_dev_status_snapshot    *ds_ptr = NULL;
  char            *buffer = NULL;
  uint32_t         buff_sz = 0;
  ...
  ds_ptr = App_malloc_pending(sizeof(T_dev_status_snapshot), MS_TO_TICKS(100));
  ...
  else
  {
    ...
    ds_ptr->sta_rssi = WIFI_get_STA_rssi();
  ...
  ds_ptr->total_up_time = g_nv_cnts.sys.accumulated_work_time;

  ds_ptr->up_time = _tx_time_get() / TX_TIMER_TICKS_PER_SECOND;
  ds_ptr->cpu_usage = (float)g_aver_cpu_usage / 10.0f;

  ds_ptr->sd_card_fs_state = g_file_system_ready;

  ds_ptr->has_intenet_connection = Is_connected_to_internet();
