...
static uint32_t Print_levels_tree(const T_NV_parameters_instance  *p_pars, T_Node *node, char *buffer, uint32_t *offset)
{
  ...
  for (uint32_t i = 0; i < p_pars->menu_items_num; i++)
  {
    if (node->level_id == p_pars->menu_items_array[i].currlev)
    {
      if (p_pars->menu_items_array[i].visible == 0) return RES_ERROR;
    }
  }
  ...
  *offset += Print_to(buffer,*offset, "\"Children\":[");
  ...
  // Не даем в конце массива оказаться запятой из-за того, например, что были пропущены некоторые ветви
