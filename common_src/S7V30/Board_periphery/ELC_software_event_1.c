...
T_ELS_soft_event_callback      ELS_soft_event1_callback;
...
/*-----------------------------------------------------------------------------------------------------
  ...
  Данное прерывание используется при организации циклического обмена данными в  внешними АЦП на плате S7V30
  и расширителями ввода вывода на плате PLCS7
...
{
  if (ELS_soft_event1_callback != 0) ELS_soft_event1_callback();
...
/*-----------------------------------------------------------------------------------------------------
  Функция вызывается каждый раз драйверами использующими прерывание ELC_EVENT_ELC_SOFTWARE_EVENT_1
  Это драйвера SCI3 (обслуживаниерасширителей ввода-вывода MCP23S17) и SCI8 (обслуживание АЦП ADS7028)

  \param callback_func
-----------------------------------------------------------------------------------------------------*/
void Set_ELS_soft_event1_callback(T_ELS_soft_event_callback func)
{
  ELS_soft_event1_callback = func;
}
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param void
-----------------------------------------------------------------------------------------------------*/
static void ELS_soft_event1_clear_interrupts(void)
{
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param void
-----------------------------------------------------------------------------------------------------*/
void ELS_soft_event1_set_and_enable_interrupts(void)
{
  ELS_soft_event1_clear_interrupts();
