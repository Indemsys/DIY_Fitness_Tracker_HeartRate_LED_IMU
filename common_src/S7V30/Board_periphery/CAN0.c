...
static void can0_error_isr(void)
{
  ...
  {
    if (can0_stat.bus_error_cnt < UINT_MAX)
    {
      can0_stat.bus_error_cnt++;
    }
  ...
  if (eifr & BIT(4)) // BORIF flag (Bus-Off Recovery Detect Flag)
  {
    ...
    can0_stat.bus_off_recovery_cnt++;
  ...
  if (eifr & BIT(5))
  {
    can0_stat.receive_overrun_cnt++;
    eifr_reset_mask ^= BIT(5);
  }

  if (eifr & BIT(6))
  {
    can0_stat.overload_frame_cnt++;
    eifr_reset_mask ^= BIT(6);
  }

  if (eifr & BIT(7))
  {
    can0_stat.bus_lock_cnt++;
    eifr_reset_mask ^= BIT(7);
  }

  if (ecsr & BIT(0))
  {
    can0_stat.stuff_error_cnt++;
    ecsr_reset_mask ^= BIT(0);
  }

  if (ecsr & BIT(1))
  {
    can0_stat.form_error_cnt++;
    ecsr_reset_mask ^= BIT(1);
  }

  if (ecsr & BIT(2))
  {
    can0_stat.ack_error_cnt++;
    ecsr_reset_mask ^= BIT(2);
  }

  if (ecsr & BIT(3))
  {
    can0_stat.crc_error_cnt++;
    ecsr_reset_mask ^= BIT(3);
  }

  if (ecsr & BIT(4))
  {
    can0_stat.bit_error_recessive_cnt++;
    ecsr_reset_mask ^= BIT(4);
  }

  if (ecsr & BIT(5))
  {
    can0_stat.bit_error_dominant_cnt++;
    ecsr_reset_mask ^= BIT(5);
  }

  if (ecsr & BIT(6))
  {
    can0_stat.ack_delimiter_cnt++;
...
static void CAN_task(ULONG arg)
{
  ...
  do
  {
    ...
    if (tx_event_flags_get(&can0_flags, 0xFFFFFFFF, TX_OR_CLEAR,&actual_flags, 1) == TX_SUCCESS)
    {
      ...
      {
        Get_hw_timestump(&can0_stat.tstart);

        can0_stat.rx_pack_count++;
        ...
        Get_hw_timestump(&can0_stat.tfinish);
        if (can0_stat.tfinish_prev.ticks != 0)
        {
          can0_stat.rx_func_period_us = Timestump_diff_to_usec(&can0_stat.tfinish_prev,&can0_stat.tfinish);
          if (can0_stat.rx_func_period_us > can0_stat.rx_func_max_period_us) can0_stat.rx_func_max_period_us = can0_stat.rx_func_period_us;
        }
        can0_stat.tfinish_prev = can0_stat.tfinish;

        can0_stat.rx_func_duration_us = Timestump_diff_to_usec(&can0_stat.tstart,&can0_stat.tfinish);
        if (can0_stat.rx_func_duration_us > can0_stat.rx_func_max_duration_us) can0_stat.rx_func_max_duration_us = can0_stat.rx_func_duration_us;
...
uint32_t Create_CAN_task(void)
{
  ...
  if (err == TX_SUCCESS)
  {
    EAPPLOG("CAN task created.");
    return RES_OK;
  }
  else
  {
    EAPPLOG("CAN creating error %d.", err);
    return RES_ERROR;
  }
}
