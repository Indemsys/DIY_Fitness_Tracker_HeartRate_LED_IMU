...
static ssp_err_t _MAX17262_read_buf(uint8_t addr, uint8_t *buf, uint32_t sz)
{
  ...
  do
  {
    res = g_sf_i2c_max17262.p_api->write(g_sf_i2c_max17262.p_ctrl,&addr, 1, true , 500); // Записываем адрес
    if (res == SSP_SUCCESS)
    {
      res = g_sf_i2c_max17262.p_api->read(g_sf_i2c_max17262.p_ctrl, buf, sz, false , 500); // Читаем данные
    }
    if (res == SSP_ERR_IN_USE)
    {
      res = g_sf_i2c_max17262.p_api->reset(g_sf_i2c_max17262.p_ctrl, 500);
    }
...
static ssp_err_t _MAX17262_write_buf(uint8_t addr, uint8_t *buf, uint32_t sz)
{
  ...
  {
    buf[0] = addr;
    res = g_sf_i2c_max17262.p_api->write(g_sf_i2c_max17262.p_ctrl, buf, sz + 1, false , 500); // Записываем адрес
    if (res == SSP_ERR_IN_USE)
    {
      res = g_sf_i2c_max17262.p_api->reset(g_sf_i2c_max17262.p_ctrl, 500);
    }
