...
static uint8_t   INT_PEND_ARRAY[SCI8_DTC_CN] =
{
...
static volatile uint8_t   *DEST_REG_ARRAY[SCI8_DTC_CN] =
{
...
static transfer_info_t SCI8_RX_dtc[] =
{
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param void
-----------------------------------------------------------------------------------------------------*/
static void SCI8_DTC_callback(void)
{
  tx_event_flags_set(&sci8_flags, SCI8_RX_COMPLETE, TX_OR);
...
{
  if (DTC_SCI8_RX_inst.p_api->open(DTC_SCI8_RX_inst.p_ctrl,DTC_SCI8_RX_inst.p_cfg)!= SSP_SUCCESS) return RES_ERROR;
  return RES_OK;
}
...
uint32_t SCI8_ADS7028_DTC_rx(void)
{
  ...
  Set_ELS_soft_event1_callback(SCI8_DTC_callback);

  // Используем регистр ELSEGRnRC0[1] для вызова прерываний по окончании циклов пересылки с помощью механизма DTC
