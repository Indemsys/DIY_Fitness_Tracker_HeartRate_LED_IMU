...
ssp_err_t BQ25619_write_buf(uint8_t addr, uint8_t *buf, uint32_t sz)
{
  ...
  {
    buf[0] = addr;
    res = g_sf_i2c_bq25619.p_api->write(g_sf_i2c_bq25619.p_ctrl, buf, sz + 1, false , 500); // Записываем адрес
    if (res == SSP_ERR_IN_USE)
    {
      res = g_sf_i2c_bq25619.p_api->reset(g_sf_i2c_bq25619.p_ctrl, 500);
    }
...
ssp_err_t BQ25619_read_buf(uint8_t addr, uint8_t *buf, uint32_t sz)
{
  ...
  do
  {
    res = g_sf_i2c_bq25619.p_api->write(g_sf_i2c_bq25619.p_ctrl,&addr, 1, true , 500); // Записываем адрес
    if (res == SSP_SUCCESS)
    {
      res = g_sf_i2c_bq25619.p_api->read(g_sf_i2c_bq25619.p_ctrl, buf, sz, false , 500); // Читаем данные
    }
    if (res == SSP_ERR_IN_USE)
    {
      res = g_sf_i2c_bq25619.p_api->reset(g_sf_i2c_bq25619.p_ctrl, 500);
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return uint32_t
-----------------------------------------------------------------------------------------------------*/
uint32_t BQ25619_set_charger_current_limit(float ilim)
{
  uint8_t v;
  uint8_t bits;
  BQ25619_VERIFY_RESULT(BQ25619_read_reg(BQ25619_CHARGE_CURRENT_LIMIT,&v));
  bits = Get_BQ25619_charge_curr_lim_bits(ilim);
  v = (v & 0xC0) | (bits & 0x3F);
  BQ25619_VERIFY_RESULT(BQ25619_write_reg(BQ25619_CHARGE_CURRENT_LIMIT,v));
  return RES_OK;
EXIT_ON_ERROR:
  return RES_ERROR;
...
/*-----------------------------------------------------------------------------------------------------
  Функция для получения значения битов на основе заданного тока зарядки

  \param charge_current

  \return uint8_t
-----------------------------------------------------------------------------------------------------*/
uint8_t Get_BQ25619_charge_curr_lim_bits(float charge_current)
{
  uint8_t bits = 0;

  // Последовательно определяем значения битов, начиная с самого значимого
  if (charge_current >= 640.0f)
  {
    bits |= (1 << 5);
    charge_current -= 640.0f;
  }
  if (charge_current >= 320.0f)
  {
    bits |= (1 << 4);
    charge_current -= 320.0f;
  }
  if (charge_current >= 160.0f)
  {
    bits |= (1 << 3);
    charge_current -= 160.0f;
  }
  if (charge_current >= 80.0f)
  {
    bits |= (1 << 2);
    charge_current -= 80.0f;
  }
  if (charge_current >= 40.0f)
  {
    bits |= (1 << 1);
    charge_current -= 40.0f;
  }
  if (charge_current >= 20.0f)
  {
    bits |= 1;
    charge_current -= 20.0f;
  }

  // Проверка на остатки, если ток зарядки не кратен 20 мА
  if (charge_current > 0.0f && charge_current <= 10.0f)
  {
    // Допустимая погрешность, округляем в большую сторону
    bits |= 1;
  }
