  ...
  #define     CAN_SPEED                      555555ul      // Скорость в системе 
...
// Меньшие значения имеют больший приоритет
  ...
  #define     CODEC_PWR_ON       R_PFS->P806PFS_b.PODR
  #define     LORA_PWR_EN        R_PFS->P808PFS_b.PODR
  #define     LORA_RESET         R_PFS->P909PFS_b.PODR

  #define     SD_CARD_PWR_ON     R_PFS->P809PFS_b.PODR
  ...
  #define     SDRAM_PWR_ON       R_PFS->P807PFS_b.PODR

  #define     EXTADC_PWR_ON      R_PFS->PB07PFS_b.PODR

  #define     VSEL1              R_PFS->P804PFS_b.PODR
  #define     VSEL2              R_PFS->P803PFS_b.PODR
  #define     VSEL3              R_PFS->P802PFS_b.PODR
...
// Спецальный макрос для поверки выходов размера переменных за заданный. Используется трюк с отрицательным размером массива.
  #define CHECK_SIZE(type, max_size) typedef char check_size_##type[(sizeof(type) <= max_size) ? 1 : -1]
...
// Биты событий обрабатываемых основным приложением
  ...
  #define EVENT_GOTO_SLEEP              BIT(5)

  #define LAST_GENERAL_EVNT_NUM         6
