...
T_bt_nv_data bt_nv @ ".stanby_ram";  // Размер структуры 0x31c (796) байт
...
static uint32_t         ble_stack_initialised;
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return uint8_t
-----------------------------------------------------------------------------------------------------*/
uint8_t BLE_stack_is_initialised(void)
{
  return ble_stack_initialised;
}
/*-----------------------------------------------------------------------------------------------------
...
uint32_t BLE_init_delete(void)
{
  ...
  EAPPLOG("BLE init task deleted. Result = %04X", res);
  return RES_OK;
}
...
_err:
  App_free(ble_init_thread_ptr);
  App_free(ble_init_stack);
  EAPPLOG("BLE init task creationn error in line %d. Result=%04X", err_line, res);
  return RES_ERROR;
}

/*-----------------------------------------------------------------------------------------------------
  Функция удаления стека BLE из памяти устройства
  Не рабочая. Требуется отладка и модификация.

  \param void
-----------------------------------------------------------------------------------------------------*/
void BLE_stack_delete(void)
{
  if (BLE_init_task_state() == BLE_INIT_TASK_FINISHED)
  {
    BLE_init_delete();
  }
  if (BLE_stack_is_initialised())
  {
