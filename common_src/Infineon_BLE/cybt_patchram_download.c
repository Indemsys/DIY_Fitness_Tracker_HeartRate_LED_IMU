...
static  uint8_t  *uncompressed_buff;
  ...
  for (uint32_t i = 0; i < INFINEON_BLOBS_CNT; i++)
  {
    ...
    if (bt_blob != NULL)
    {
      ...
      if (uncompressed_buff != NULL)
      {
        ...
        // Удалять здесь буфер uncompressed_buff нельзя, поскольку загрузка прооисходит асинхронно 
      }
...
void bt_fw_download_complete_cback(cybt_prm_status_t status)
{
  ...
  // Освобождаем здесь буфер использованный для загрузки патча 
