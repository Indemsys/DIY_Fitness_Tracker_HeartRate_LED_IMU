...
uint8_t                       mqtt_client_stack[MQTT_CLIENT_STACK_SIZE] BSP_PLACE_IN_SECTION_V2(".stack.mqtt_thread")BSP_ALIGN_VARIABLE_V2(BSP_STACK_ALIGNMENT);
...
{
  Send_wifi_event(NET_FLG_MQTT_MSG);
}
...
uint32_t Net_MQTT_client_create(NX_IP  *ip)
{
  ...
  if (ivar.mqtt_enable == 0) return RES_ERROR;
...
static void _mqtt_client_id_callback(char *client_id, uint32_t *client_id_length)
{
  ...
  vlen = strlen((char const *)ivar.mqtt_client_id);
  if (len > vlen) len = vlen;

  strncpy(client_id,(char const *) ivar.mqtt_client_id, len);
...
uint32_t Net_mqtt_client_connect(void)
{
  ...
  res = Str_to_IP_v4((char const *)ivar.mqtt_server_ip,(uint8_t *)&ip_addr.nxd_ip_address.v4);
  if (res != RES_OK)
  {
    res =  DNS_get_host_address((UCHAR *)ivar.mqtt_server_ip,&addr, MS_TO_TICKS(2000));
    if (res == NX_SUCCESS)
    {
  ...
  res = nxd_mqtt_client_login_set(&mqtt_client, (CHAR *)ivar.mqtt_user_name, strlen((char const *)ivar.mqtt_user_name), (CHAR *)ivar.mqtt_password, strlen((char const *)ivar.mqtt_password));
  if (res != NXD_MQTT_SUCCESS)
  {
    if (mqtt_connection_attempt_cnt < 5)
    {
  ...
  res  = nxd_mqtt_client_connect(&mqtt_client,&ip_addr, ivar.mqtt_server_port,  MQTT_KEEP_ALIVE_TIMER, clean_session, NX_WAIT_FOREVER);
  if (res != NXD_MQTT_SUCCESS)
  {
    if (mqtt_connection_attempt_cnt < 5)
    {
