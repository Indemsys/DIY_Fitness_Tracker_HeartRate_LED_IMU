...
#define MAX_HEADER_LEN        27
static  char          g_mqtt_message_buffer[MAX_MQTT_PAYLOAD_LEN + MAX_HEADER_LEN];
...
{
  UINT       res;
  res = nxd_mqtt_client_unsubscribe(&mqtt_client, g_cpu_id_str, strlen(g_cpu_id_str));
  if (res == NXD_MQTT_SUCCESS)
  {
    res = nxd_mqtt_client_subscribe(&mqtt_client, g_cpu_id_str, strlen(g_cpu_id_str), MQTT_QOS1);
  }
...
static uint32_t _MQTTMC_send_file_by_handler(FX_FILE *file_handler, uint8_t compress_flag)
{
  ...
  do
  {
    ...
    memcpy(&g_mqtt_message_buffer[1],&g_cpu_id,16);            // Записываем уникальный идентификатор устройства
...
static void _MQTTMC_reset_stream(void)
{
  ...
  if (msm.data_file.fx_file_id == FX_FILE_ID)
  {
    ...
    if (msm.file_transfer)
    {
      ...
      msm.err_cnt++;
      if (_MQTTMC_reset_subscribe() != NXD_MQTT_SUCCESS) Send_flag_to_background(APP_DO_SYSTEM_RESTART);
...
{
  uint32_t res = RES_ERROR;
  res = Serialze_settings_schema_to_JSON_file(&ivar_inst,PARAMS_SCHEMA_FILE_NAME, JSON_ENSURE_ASCII | JSON_COMPACT);
  if (res == RES_OK)
  {
...
{
  uint32_t res = RES_ERROR;
  res = Serialze_settings_to_JSON_file(&ivar_inst,PARAMS_VALUES_FILE_NAME, JSON_ENSURE_ASCII | JSON_COMPACT);
  if (res == RES_OK)
  {
...
void Net_MQTT_client_task(NX_IP   *ip_ptr)
{
  ...
  if (Is_mqtt_client_created() == 1)
  {
    ...
    if ( Wait_net_event(NET_FLG_MQTT_MSG, &actual_flags, TX_NO_WAIT)==TX_SUCCESS)
    {
...
uint32_t _MQTTMC_execute_command(void)
{
  ...
  case OPCODE_RESET_TO_DEFAULT:
    ...
    MQTTMC_Send_Ack(RES_OK);
    Return_def_params(MODULE_PARAMS);
  ...
  case OPCODE_REQUEST_STATE_INFO:
    // Состояние посылаем не чаще одного раза в секунду чтобы не зафлудить канал
    {
      uint32_t t;
      t = tx_time_get();
      if ((t - last_state_req_t) > MS_TO_TICKS(1000))
      {
...
void MQTTMC_messages_processor(void)
{
  ...
  do
  {
    ...
    if (res == NXD_MQTT_SUCCESS)
    {
      ...
      if (strcmp(g_mqtt_topic_buffer, g_cpu_id_str) == 0)
      {
        if (message_length > 0)
        {
