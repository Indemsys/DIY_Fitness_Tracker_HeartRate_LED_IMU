...
#define MAX_BSS_INFO_STR_LEN  128
...
uint32_t WIFI_STA_accept_connection_to_AP(void)
{
  ...
  if (sta_connection_active) return res;
...
uint32_t WIFI_STA_accept_disconnection_from_AP(void)
{
  ...
  if (sta_connection_active == 0) return res;
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return uint32_t
-----------------------------------------------------------------------------------------------------*/
static uint32_t WIFI_Log_AP_BSS_info(void)
{
  uint32_t res;
  wl_bss_info_t bi;
  char     str[MAX_BSS_INFO_STR_LEN+1];
  uint32_t str_len;
  whd_driver_t whd_driver = g_whd_driver;

  res = whd_wifi_get_bss_info(g_prim_whd_intf_ptr,&bi);

  str_len = 0;
  str_len += snprintf(&str[str_len], MAX_BSS_INFO_STR_LEN - str_len, "WIFI STA: RSSI=%d dBm, Phy noise=%d dBm, SNR=%d, ",bi.RSSI, bi.phy_noise, bi.SNR);

  // Определение диапазона
  if (CHSPEC_IS2G(bi.chanspec))
  {
    str_len += snprintf(&str[str_len], MAX_BSS_INFO_STR_LEN - str_len, " 2.4 GHz");
  }
  else if (CHSPEC_IS5G(bi.chanspec))
  {
   str_len += snprintf(&str[str_len], MAX_BSS_INFO_STR_LEN - str_len, " 5 GHz");
  }
  else
  {
    str_len += snprintf(&str[str_len], MAX_BSS_INFO_STR_LEN - str_len, "? GHz\n");
  }

  // Получение номера канала
  uint8_t channel = CHSPEC_CHANNEL(bi.chanspec);

  str_len += snprintf(&str[str_len], MAX_BSS_INFO_STR_LEN - str_len, " Ch=%d", channel);
