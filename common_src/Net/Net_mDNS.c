...
static NX_IP                  *mdns_server_ip_ptr;
...
uint32_t Net_mDNS_server_create(NX_IP *ip_ptr)
{
  ...
  if (mdns_server_ip_ptr != 0) return RES_ERROR;
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return uint32_t
-----------------------------------------------------------------------------------------------------*/
uint32_t Net_mDNS_server_delete(void)
{
  if (mdns_server_ip_ptr == 0) return RES_ERROR;
  nx_mdns_delete(&mdns);
  mdns_server_ip_ptr = 0;
  return RES_OK;
}
