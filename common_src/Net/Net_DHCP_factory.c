...
static NX_IP                  *dhcp_client_ip_ptr;
static NX_IP                  *dhcp_server_ip_ptr;
...
uint32_t Net_DHCP_server_create(NX_IP *ip_ptr)
{
  ...
  if (dhcp_server_ip_ptr != 0) return RES_ERROR;
...
uint32_t Net_DHCP_client_create(NX_IP *ip_ptr)
{
  ...
  if (dhcp_client_ip_ptr != 0) return RES_ERROR;
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return uint32_t
-----------------------------------------------------------------------------------------------------*/
uint32_t Net_DHCP_client_delete(void)
{
  if (dhcp_client_ip_ptr == 0) return RES_ERROR;

  nx_dhcp_stop(&net_dhcp_client);
  nx_dhcp_delete(&net_dhcp_client);
  dhcp_client_ip_ptr = 0;
  return RES_OK;
}
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param void
-----------------------------------------------------------------------------------------------------*/
uint32_t Net_DHCP_server_delete(void)
{
  if (dhcp_server_ip_ptr == 0) return RES_ERROR;

  nx_dhcp_server_stop(&net_dhcp_server);
  nx_dhcp_server_delete(&net_dhcp_server);
  dhcp_server_ip_ptr = 0;
  return RES_OK;
}
