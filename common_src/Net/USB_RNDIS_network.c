...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param void
-----------------------------------------------------------------------------------------------------*/
static void RNDIS_diconnect_handler(void)
{
...
UINT Register_rndis_class(void)
{
  ...
  USB_device_set_suspend_callback(RNDIS_diconnect_handler);
...
/*-----------------------------------------------------------------------------------------------------
  ...
  !!! При отключении кабеля функция не вызывается. Функция вызывается при повтороном подключении кабеля

  Цепочка вызовов до этой функции:
  usbhs_usb_int_resume_isr ->
  ux_dcd_synergy_interrupt_handler ->
  _ux_device_stack_disconnect -> class_ptr ->
  ux_slave_class_entry_function(&class_command) ->
  _ux_device_class_rndis_deactivate ->rndis ->
  ux_slave_class_rndis_parameter.ux_slave_class_rndis_instance_deactivate(rndis) ->
  RNDIS_instance_deactivate_callback
...
uint32_t RNDIS_to_start(void)
{
  ...
  if (net_dev->ux_network_device_usb_link_up == 0)
  {
