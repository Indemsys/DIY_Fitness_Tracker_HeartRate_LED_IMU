...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return uint32_t
-----------------------------------------------------------------------------------------------------*/
int32_t WIFI_get_STA_rssi(void)
{
  return g_sta_rssi;
}
...
static void WIFI_log_module_info(void)
{
  ...
  NETLOG("WiFi Access Point Channel = %d, SSID = %s, CNTRY = %08X", ivar.wifi_ap_channel, g_wifi_ap_ssid, (uint32_t)g_whd_init_cfg.country);
...
-----------------------------------------------------------------------------------------------------*/
static void* WIFI_STA_event_handler(whd_interface_t ifp, const whd_event_header_t *event_header, const uint8_t *event_data, void *handler_user_data)
{
  NETLOG("WIFI STA. STA Event: %d (%s) Reason: %d, Status: %d ", event_header->event_type, WHD_event_to_string(event_header->event_type), event_header->reason, event_header->status);

  if  ((event_header->event_type == (uint32_t)WLC_E_LINK) && (event_header->reason == WLC_E_REASON_INITIAL_ASSOC))
  {
    ...
    if (ifp == g_prim_whd_intf_ptr)
    {
      ...
      NETLOG("WIFI STA. Connection to remote AP      %02X:%02X:%02X:%02X:%02X:%02X",event_header->addr.octet[0], event_header->addr.octet[1],event_header->addr.octet[2], event_header->addr.octet[3], event_header->addr.octet[4], event_header->addr.octet[5]);
    }
  }
  if (((event_header->event_type == (uint32_t)WLC_E_LINK) &&
       ((event_header->reason == WLC_E_REASON_DEAUTH) || (event_header->reason == WLC_E_REASON_DISASSOC) || (event_header->reason ==  WLC_E_REASON_BCNS_LOST) || (event_header->reason == WLC_E_REASON_FAST_ROAM_FAILED)))
      ||  ((event_header->event_type == (uint32_t)WLC_E_DEAUTH_IND) && (event_header->reason == 7)))
  {
    ...
    if (ifp == g_prim_whd_intf_ptr)
    {
      ...
      NETLOG("WIFI STA. Disconnection from remote AP %02X:%02X:%02X:%02X:%02X:%02X",event_header->addr.octet[0], event_header->addr.octet[1],event_header->addr.octet[2], event_header->addr.octet[3], event_header->addr.octet[4], event_header->addr.octet[5]);
    }
  }
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return void* - Ссылка на пользовательские данные
-----------------------------------------------------------------------------------------------------*/
static void* WIFI_AP_event_handler(whd_interface_t ifp, const whd_event_header_t *event_header, const uint8_t *event_data, void *handler_user_data)
{
  NETLOG("WIFI AP. AP Event: %d (%s) Reason: %d, Status: %d ", event_header->event_type, WHD_event_to_string(event_header->event_type), event_header->reason, event_header->status);
  ...
  if (event_header->event_type == (uint32_t)WLC_E_AUTHORIZED)
  {
    ...
    if (ifp == g_secd_whd_intf_ptr)
    {
      ...
      NETLOG("WIFI AP. Remote STA connection        %02X:%02X:%02X:%02X:%02X:%02X",event_header->addr.octet[0], event_header->addr.octet[1],event_header->addr.octet[2], event_header->addr.octet[3], event_header->addr.octet[4], event_header->addr.octet[5]);
    }
  }
  if ((event_header->event_type == (uint32_t)WLC_E_WAI_STA_EVENT) || (event_header->event_type == (uint32_t)WLC_E_DEAUTH))
  {
    ...
    if (ifp == g_secd_whd_intf_ptr)
    {
      ...
      NETLOG("WIFI AP. Remote STA disconnection     %02X:%02X:%02X:%02X:%02X:%02X",event_header->addr.octet[0], event_header->addr.octet[1],event_header->addr.octet[2], event_header->addr.octet[3], event_header->addr.octet[4], event_header->addr.octet[5]);
    }
  }
  return handler_user_data;
}
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return wwd_result_t
-----------------------------------------------------------------------------------------------------*/
whd_result_t WIFI_wifi_set_AP_max_associations(uint32_t max_assoc)
{
  uint32_t     res;
...
-----------------------------------------------------------------------------------------------------*/
static uint32_t WIFI_get_connection_quality(int32_t *rssi)
{
  uint32_t res;
  res = whd_wifi_get_rssi(g_prim_whd_intf_ptr, rssi);
...
uint32_t WIFI_init_module(void)
{
  ...
  WIFI_wifi_set_AP_max_associations(1);

  res = whd_wifi_set_event_handler(g_prim_whd_intf_ptr, (uint32_t *)events_list,WIFI_STA_event_handler,NULL,&whd_sta_event_index);
  if (res != WHD_SUCCESS)
  {
    whd_wifi_off(g_prim_whd_intf_ptr);
    err_line = __LINE__;
  ...
  res = whd_wifi_set_event_handler(g_secd_whd_intf_ptr, (uint32_t *)events_list,WIFI_AP_event_handler,NULL,&whd_ap_event_index);
  if (res != WHD_SUCCESS)
  {
    whd_wifi_off(g_prim_whd_intf_ptr);
    err_line = __LINE__;
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return uint32_t
-----------------------------------------------------------------------------------------------------*/
uint32_t WIFI_deinit_module(void)
{
  if (ivar.en_wifi_ap) whd_wifi_stop_ap(g_secd_whd_intf_ptr);
  whd_wifi_off(g_prim_whd_intf_ptr);
  whd_bus_sdio_detach(g_whd_driver);
  whd_sdpcm_quit(g_whd_driver);
  whd_deinit(g_prim_whd_intf_ptr);
...
static void Thread_wifi_manager(ULONG initial_input)
{
  ...
  do
  {
    if (Wait_wifi_event(0xFFFFFFFF,&actual_flags, 100) == TX_SUCCESS)
    {
    ...
    WIFI_get_connection_quality(&g_sta_rssi);
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return uint32_t
-----------------------------------------------------------------------------------------------------*/
uint32_t Thread_WiFi_manager_delete(void)
{
  tx_thread_terminate(&wifi_man_thread);
  tx_thread_delete(&wifi_man_thread);
  return RES_OK;
}
