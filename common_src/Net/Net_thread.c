...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return uint32_t
-----------------------------------------------------------------------------------------------------*/
uint32_t Is_network_connected(void)
{
  if (WIFI_AP_link_state() != NULL) return RES_OK;
  if (WIFI_STA_link_state() != NULL) return RES_OK;
  if (RNDIS_link_state() != NULL) return RES_OK;
  if (ECM_host_link_state() != NULL) return RES_OK;
  return RES_ERROR;
}
...
uint32_t Wait_net_event(uint32_t event_flags, uint32_t *actual_flags, uint32_t timeout_ms)
{
  ...
  else if (timeout_ms == TX_WAIT_FOREVER)
  {
    return tx_event_flags_get(&net_flag, event_flags, TX_OR_CLEAR, (ULONG *)actual_flags,  TX_WAIT_FOREVER);
  }
  else
  {
...
/*-----------------------------------------------------------------------------------------------------
  Функция удаления сетевого стека рабатающего через Wi-Fi модуль
  Функцияо отлаженна, но может конфликтовать с запущеным стеком BLE работающем на том же Wi-Fi модуле.

  \param void

  \return uint32_t
-----------------------------------------------------------------------------------------------------*/
uint32_t Delete_Net_stack(void)
{
  Net_HTTP_server_delete();
  Net_DHCP_client_delete();
  Net_DNS_client_delete();
  Net_DHCP_server_delete();
  Net_mDNS_server_delete();
  VT100_Telnet_sessions_delete();
  TELNET_server_delete();
  Net_FTP_server_delete();
  Net_MQTT_client_delete();
  Net_FreeMaster_server_delete();
  SNTP_client_delete();
  #ifdef ENABLE_MATLAB_CONNECTION
  MATLAB_server_delete();
  #endif

  tx_thread_terminate(&net_thread);
  tx_event_flags_delete(&net_flag);
  tx_thread_delete(&net_thread);
  nx_ip_delete(&net_ip);
  Net_packet_pool_delete();

  WIFI_deinit_module();
  Thread_WiFi_manager_delete();

  _nx_secure_tls_shutdown();
  g_sf_crypto.p_api->close(g_sf_crypto.p_ctrl);
