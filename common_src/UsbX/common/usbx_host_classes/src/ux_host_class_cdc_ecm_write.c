...
UINT  _ux_host_class_cdc_ecm_write(VOID *cdc_ecm_class, NX_PACKET *packet)
{
  ...
  if (cdc_ecm->ux_host_class_cdc_ecm_link_state == UX_HOST_CLASS_CDC_ECM_LINK_STATE_UP)
  {
    ...
    if (cdc_ecm->ux_host_class_cdc_ecm_xmit_queue_head == UX_NULL)
    {
      ...
      if (packet->nx_packet_next != UX_NULL)
      {
        ...
        {
          cdc_ecm->ux_host_class_cdc_ecm_xmit_buffer = _ux_utility_memory_allocate(UX_NO_ALIGN, UX_CACHE_SAFE_MEMORY, UX_HOST_CLASS_CDC_ECM_NX_PAYLOAD_SIZE);
          if (cdc_ecm->ux_host_class_cdc_ecm_xmit_buffer == UX_NULL)
          {
  ...
  if (cdc_ecm->ux_host_class_cdc_ecm_bulk_out_transfer_waiting_for_check_and_arm_to_finish == UX_TRUE) _ux_host_semaphore_put(&cdc_ecm->ux_host_class_cdc_ecm_bulk_out_transfer_waiting_for_check_and_arm_to_finish_semaphore);
