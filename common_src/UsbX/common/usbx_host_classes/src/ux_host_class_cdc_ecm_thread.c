...
VOID  _ux_host_class_cdc_ecm_thread(ULONG parameter)
{
  ...
  while (1)
  {
    ...
    if (cdc_ecm->ux_host_class_cdc_ecm_link_state == UX_HOST_CLASS_CDC_ECM_LINK_STATE_PENDING_UP)
    {
      ...
      while ((cdc_ecm->ux_host_class_cdc_ecm_link_state == UX_HOST_CLASS_CDC_ECM_LINK_STATE_UP) && (cdc_ecm->ux_host_class_cdc_ecm_device->ux_device_state == UX_DEVICE_CONFIGURED))
      {
        ...
        status =  nx_packet_allocate(cdc_ecm->ux_host_class_cdc_ecm_packet_pool,&packet,NX_RECEIVE_PACKET, UX_MS_TO_TICK(UX_HOST_CLASS_CDC_ECM_PACKET_POOL_WAIT));
        ...
        if (status == NX_SUCCESS)
        {
          ...
          if (packet_buffer_size < UX_HOST_CLASS_CDC_ECM_NX_PAYLOAD_SIZE)
          {
            ...
            {
              cdc_ecm->ux_host_class_cdc_ecm_receive_buffer = _ux_utility_memory_allocate(UX_SAFE_ALIGN, UX_CACHE_SAFE_MEMORY,UX_HOST_CLASS_CDC_ECM_NX_PAYLOAD_SIZE);
              if (cdc_ecm->ux_host_class_cdc_ecm_receive_buffer == UX_NULL)
              {
          ...
          if (cdc_ecm->ux_host_class_cdc_ecm_link_state == UX_HOST_CLASS_CDC_ECM_LINK_STATE_UP)
          {
            ...
            if (cdc_ecm->ux_host_class_cdc_ecm_bulk_in_transfer_waiting_for_check_and_arm_to_finish == UX_TRUE) _ux_host_semaphore_put(&cdc_ecm->ux_host_class_cdc_ecm_bulk_in_transfer_waiting_for_check_and_arm_to_finish_semaphore);
            ...
            if (status == UX_SUCCESS)
            {
              ...
              if (transfer_request->ux_transfer_request_completion_code == UX_SUCCESS)
              {
                      ...
                      transfer_request->ux_transfer_request_data_pointer)
                {
                  ...
                  status = nx_packet_data_append(packet,transfer_request->ux_transfer_request_data_pointer,transfer_request->ux_transfer_request_actual_length,cdc_ecm->ux_host_class_cdc_ecm_packet_pool,UX_MS_TO_TICK(UX_HOST_CLASS_CDC_ECM_PACKET_POOL_WAIT));
                  if (status != NX_SUCCESS)
                  {
                    /* Release packet.  */
                    nx_packet_release(packet);
