...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param status
-----------------------------------------------------------------------------------------------------*/
static void Rename_firmware_file(uint8_t status, char *file_name)
{
  ...
  {
    strncpy(new_name, file_name, 63- strlen(FIRMWARE_FILE_NAME_RENAMED_SFX));
    strcat(new_name, FIRMWARE_FILE_NAME_RENAMED_SFX);
  }
  else if (status == STATUS_LOADING_FIRWARE_FAIL)
  {
    strncpy(new_name, file_name, 63- strlen(FIRMWARE_LOADING_FAIL_RENAME_SFX));
    strcat(new_name, FIRMWARE_LOADING_FAIL_RENAME_SFX);
  }
  else if (status == STATUS_FLASHING_FIRMWARE_FAIL)
  {
    strncpy(new_name, file_name, 63- strlen(FIRMWARE_PROGRAMING_FAIL_RENAME_SFX));
    strcat(new_name, FIRMWARE_PROGRAMING_FAIL_RENAME_SFX);
  }
  else
  {
    return;
  }
  fx_file_delete(&fat_fs_media,new_name);
  fx_file_rename(&fat_fs_media, file_name, new_name);
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return uint32_t
-----------------------------------------------------------------------------------------------------*/
static uint32_t Open_firmware_file(FX_FILE *file_ptr, char *file_name)
{
  ULONG     actual_size;
  uint32_t  crc32;

  if (fx_file_open(&fat_fs_media, file_ptr, file_name,  FX_OPEN_FOR_READ) != FX_SUCCESS)
  {
  ...
  if (file_ptr->fx_file_current_file_size <= (sizeof(T_file_header) + sizeof(T_data_header)))
  {
    ...
    Rename_firmware_file(STATUS_LOADING_FIRWARE_FAIL, file_name);
...
static uint32_t Decrypt_image_sign(FX_FILE *file_ptr, T_file_header  *fh_ptr, T_file_sign *fs_prt)
{
  ...
  data_block =  SDRAM_malloc(signed_block_size * 2); // Выделяем место сразу для входного блока и для выходного
...
static uint32_t Decrypt_image_data_header(FX_FILE *file_ptr, T_file_header  *fh_ptr, T_file_sign *fs_prt, T_data_header *dh_ptr)
{
  ...
  cryptotext = (uint8_t *)SDRAM_malloc(cryptotext_sz);
...
static uint32_t Check_encrypted_file_digest(FX_FILE *file_ptr, T_file_header  *fh_ptr, T_file_sign *fs_prt, T_data_header *dh_ptr)
{
  ...
  data_buf = (uint8_t *)SDRAM_malloc(data_buf_sz);
...
static uint32_t Flash_image(FX_FILE *file_ptr,  T_file_sign *fs_prt, T_file_header  *fh_ptr, T_data_header *dh_ptr)
{
  ...
  buf1 = (uint8_t *)SDRAM_malloc(FILE_CHANK_SZ + 10 * 1024);
  if (buf1 == NULL) goto error;
  buf2 = (uint8_t *)SDRAM_malloc(FILE_CHANK_SZ + 10 * 1024);
  ...
  do
  {
    ...
    do
    {
      ...
      {
        section_sz = CODE_FLASH_EBLOCK_SMALL_SZ;
      }
      else
      {
...
//  // Если данные начианюся с нуля, то это бутлодер и контрольную сумму не проверяем
//  if (dh_ptr->start_address!=0)
//  {
...
/*-----------------------------------------------------------------------------------------------------
  Считывание, распаковка, расшифровка и программирования бинарного образа во Flash
...
uint32_t Load_and_Flash_Image_File(char *file_name)
{
  ...
  if (Open_firmware_file(&file, file_name) != RES_OK) return RES_ERROR;
  ...
  {
    fx_file_close(&file);
    Rename_firmware_file(STATUS_FLASHING_FIRMWARE_FAIL, file_name);
    return RES_ERROR;
  }


  fx_file_close(&file);
  Rename_firmware_file(STATUS_FILE_PROGRAMED, file_name);
...
{
  uint32_t res  = 0;
  uint32_t n = image_sz/CODE_FLASH_EBLOCK_SZ;
  Switch_Flash_driver_to_no_bgo();
  __disable_interrupt();
  res = Flash_erase_block(strt_address, n);
  __enable_interrupt();
  Switch_Flash_driver_to_bgo();
  return  res;
}
...
uint32_t Integrity_check_App_firmware(uint32_t start_address, uint32_t image_sz)
{
  ...
  flash_crc = Get_CRC32((uint8_t *)(start_address), image_sz - 4);
  memcpy(&img_crc, (void *)(start_address + image_sz - 4),4);
  Get_hw_timestump(&t2);
...
/*-----------------------------------------------------------------------------------------------------
  Установка аттрибутов указывающих начальному загрузчику перейти к выполнению приложения без предварительных проверок
