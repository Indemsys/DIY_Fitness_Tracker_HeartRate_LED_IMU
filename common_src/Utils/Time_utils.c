...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return int32_t
-----------------------------------------------------------------------------------------------------*/
int32_t  Get_date_time_str(char *str, uint32_t max_str_len)
{
  rtc_time_t  curr_time;
  rtc_cbl.p_api->calendarTimeGet(rtc_cbl.p_ctrl,&curr_time);
  curr_time.tm_mon++;
  return snprintf(str, max_str_len, "%04d.%02d.%02d  %02d:%02d:%02d",curr_time.tm_year + 1900, curr_time.tm_mon, curr_time.tm_mday, curr_time.tm_hour, curr_time.tm_min, curr_time.tm_sec);
