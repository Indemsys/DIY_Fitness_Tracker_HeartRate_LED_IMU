...
T_file_system_init_results   fs_res;
...
uint32_t Init_SD_card_file_system(void)
{
  ...
  // Очищаем эти переменные вручную поскольку они не очищаются стартапным кодом, поскольку находятся в секции .fs_mem
  memset(fs_memory, 0, sizeof(fs_memory));
  memset(&g_sf_el_fx0_ctrl, 0, sizeof(g_sf_el_fx0_ctrl));
...
uint32_t Set_SD_card_password(uint8_t operation, char *password, uint32_t password_len, sdmmc_priv_card_status_t  *p_response)
{
  ...
  if ((res1 == FX_SUCCESS) || (res1 == FX_MEDIA_NOT_OPEN))
  {
    ...
    if ((res == RES_OK) || ((ust->lock_detected==1) && (ust->unlock_executed == 0)))
    {
      ...
      if (res == RES_OK)
      {
        EAPPLOG("SD card operation %s done successfully", Get_sd_password_op_str(operation));
      }
      else
      {
        EAPPLOG("SD card operation %s error",  Get_sd_password_op_str(operation));
      }
    }
    SD_card_close();
