...
static void   _Print_AB1815_test_header(void)
{
  ...
  MPRINTF("            [W] - write register, [N] - read NV RAM,        [C] - Clear chip RAM\n\r");
  MPRINTF("----------------------------------------------------------------------\n\r");
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return uint32_t
-----------------------------------------------------------------------------------------------------*/
static uint32_t _AB1815_read_nv_ram(void)
{
  uint32_t  n;
  uint8_t   *ram_buf;
  ...
  GET_MCBL;
  n = 0;
  ram_buf = App_malloc(AB1815_NVRAM_SZ);
  if (ram_buf != NULL)
  {
    if (AB1815_read_chip_RAM(0, ram_buf, AB1815_NVRAM_SZ) == RES_OK)
    {
