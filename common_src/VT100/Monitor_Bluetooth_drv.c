...
static void Init_monitor_bluetooth_driver(T_vt100_man_msg *msg_ptr)
{
  ...
  if (bt_cbl.taken == 0)
  {
    ...
    if (res == RES_OK)
    {
      ...
      EAPPLOG("Bluetooth VT100 task %d created", bt_cbl.vt100_task_instance_index);
    }
    else
    {
      goto err_exit;
...
err_exit:
  ...
  bt_cbl.taken = 0;
  EAPPLOG("Bluetooth VT100 task creating error");
  return;
}
...
static void DeInit_monitor_bluetooth_driver(T_vt100_man_msg *msg_ptr)
{
  ...
  EAPPLOG("Bluetooth VT100 task %d deleted", bt_cbl.vt100_task_instance_index);
}
