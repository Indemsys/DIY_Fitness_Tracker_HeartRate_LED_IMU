...
void Do_system_info(uint8_t keycode)
{
  ...
  for (uint32_t i=0;i<2;i++)
  {
    ...
    {
    case SETT_OK:
      MPRINTF("Module settings area %d Ok. Start cond.=%d. Size=%d bytes, Write cnt =%d\r\n", i, sstate.area_start_condition[i], sstate.area_sz[i], sstate.area_wr_cnt[i]);
      break;
    case SETT_WRONG_SIZE:
      MPRINTF("Module settings area %d Error. Start cond.=%d. Wrong size=%d bytes.\r\n", i,  sstate.area_start_condition[i], sstate.area_sz[i]);
      break;
    case SETT_WRONG_CRC:
      MPRINTF("Module settings area %d Error. Start cond.=%d. Wrong CRC. Size=%d bytes.\r\n", i,  sstate.area_start_condition[i], sstate.area_sz[i]);
      break;
    case SETT_WRONG_CHECK:
      MPRINTF("Module settings area %d Error. Start cond.=%d. Check fault.\r\n",  sstate.area_start_condition[i], i);
      break;
    case SETT_IS_BLANK:
      MPRINTF("Module settings area %d Error. Area is blank.\r\n",  sstate.area_start_condition[i]);
      break;
    }
  }
      ...
      case 's':
        {
          ...
          res = ParamsSchema_serialize_to_file(MODULE_PARAMS,PARAMS_SCHEMA_FILE_NAME);
          if (res==RES_OK)
          {
