...
static NX_IP    *telnet_server_ip_ptr;
/*-----------------------------------------------------------------------------------------------------
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return uint32_t
-----------------------------------------------------------------------------------------------------*/
uint32_t Net_TELNET_server_create(NX_IP *ip_ptr)
{
  UINT    err;

  if (ivar.en_telnet == 0) return RES_ERROR;
  if (telnet_server_ip_ptr !=  0)  return RES_OK;
...
{
  if (telnet_server_ip_ptr ==  0)  return RES_ERROR;
  nx_telnet_server_stop(&telnet_server);
  nx_telnet_server_delete(&telnet_server);
  NET_LOG("Telent server: Deleted");
  return RES_OK;
}
