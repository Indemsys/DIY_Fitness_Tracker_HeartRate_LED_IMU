...
static void Task_VT100(ULONG arg)
{
  ...
  do
  {
    ...
    if (mcbl->menu_trace[mcbl->menu_nesting] == &MENU_MAIN)
    {
      ...
      MPRINTF(VT100_CLR_LINE"Up time      : %d s. Total operating time: %d s. Total reboot count: %d, CPU usage %d%% \r\n",
              _tx_time_get() / TX_TIMER_TICKS_PER_SECOND,
              g_nv_cnts.sys.accumulated_work_time,
              g_nv_cnts.sys.reboot_cnt,
              g_aver_cpu_usage / 10);
...

      {
        ...
        App_get_SDRAM_pool_statistic(&ab2,&fragments);
        if (g_file_system_ready)
        {
          MPRINTF(VT100_CLR_LINE"Chip RAM free: %d bytes, SDRAM free: %d bytes, SD card: Operating\r\n", ab1, ab2);
        }
        else
        {
          MPRINTF(VT100_CLR_LINE"Chip RAM free: %d bytes, SDRAM free: %d bytes, SD card: Non-Operating (0X%04X)\r\n", ab1, ab2, fs_res.fx_media_open_result);
        }
      }

      {
        ...
        if (ip_interface != NULL)
        {
          ...
          MPRINTF(" SSID= %s, RSSI=%d dBm",WIFI_STA_get_current_SSID(), WIFI_get_STA_rssi());
...
/*-----------------------------------------------------------------------------------------------------
  Удаляем все задачи связанные с TELNET
